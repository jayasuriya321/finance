...
export default function Expenses() {
  ...
  const { currency } = useCurrency(); // ✅ Use currency context
  ...
  const recurringData = useMemo(() => {
    ...
    recurring.forEach((r) => {
      ...
      data[r.category] = (data[r.category] || 0) + Number(r.amount);
    });
    return Object.entries(data).map(([name, value]) => ({ name, value }));
  }, [recurring]);
  ...
  return (
    <DashboardLayout>
      ...
      <h1 className="text-3xl font-bold mb-6 text-gray-700 dark:text-gray-100">
        Expenses Dashboard
      </h1>
      ...
      {budgetWarning && (
        <div className="mb-4 p-3 rounded text-sm border bg-yellow-100 text-yellow-800 border-yellow-300">
          {budgetWarning}
        </div>
      )}
      ...
      {/* ===== Budget vs Actual ===== */}
      <div className="bg-white dark:bg-gray-800 rounded shadow p-4 mb-6 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mb-6 transition-colors duration-300">
        ...
        >
          <input
            ...
            className="p-2 border rounded dark:bg-gray-700 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500"
            required
          />
          <select
            name="category"
          ...
          <button
            ...
            className="col-span-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 transition flex justify-center items-center disabled:opacity-60"
          >
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center mb-4 gap-2">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <input
            ...
            className="p-2 border rounded w-full sm:w-64 dark:bg-gray-700 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500"
          />
        </div>
        <div className="flex gap-2 mt-2 sm:mt-0">
          <button
            onClick={() => exportToCSV(filteredExpenses, "expenses")}
            className="flex items-center gap-1 bg-indigo-500 text-white px-3 py-1.5 rounded hover:bg-indigo-600 transition"
          >
      ...
      {/* ===== Expenses Table ===== */}
      <div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-x-auto transition-colors duration-300">
...
function ChartBox({ title, data }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded shadow p-4 transition-colors duration-300">
      <h3 className="font-semibold text-gray-700 dark:text-gray-200 mb-2">
        {title}
      </h3>
